...
<AnimatePresence>
  {isOpen && (
    <motion.div 
      key="mobile-menu"
      className="md:hidden bg-[#008EAA] text-white p-6 fixed top-0 right-0 h-full w-2/3 flex flex-col items-center gap-4 z-50 shadow-lg"
      initial={{ x: '100%' }}          
      animate={{ x: 0 }}               
      exit={{ x: '100%' }}             
    ...
    >
      ...
      <Link
        ...
        onClick={() => setIsOpen(false)}
      >
        <button className="mt-6 px-6 py-3 bg-[#25D366] text-white rounded-lg font-semibold flex items-center gap-2 hover:bg-[#1EBE5F] transition">
